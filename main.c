...
/* Interrupt configs */
const cy_stc_sysint_t INTR_CFG_USER_BTN =
{
...
{
    .intrSrc = ((NvicMux2_IRQn << CY_SYSINT_INTRSRC_MUXIRQ_SHIFT) | TCPWM_TIMER_IRQ),
    .intrPriority = 7UL
};
...
/*------------------------------------------------Function Prototypes------------------------------------------------*/
/*********************************************************************************************************************/
static void isrUserButton(void);
...
int main(void)
{
    ...
    /* Initialize user button to change scenes */
    ...
    if (Cy_SysInt_Init(&INTR_CFG_USER_BTN, isrUserButton) != CY_SYSINT_SUCCESS)
    {
        CY_ASSERT(0);
    }
    NVIC_ClearPendingIRQ((IRQn_Type)INTR_CFG_USER_BTN.intrSrc);
    NVIC_EnableIRQ((IRQn_Type) NvicMux2_IRQn);
    
    /* Initialize timer to toggle LED */
    if (Cy_TCPWM_Counter_Init(TCPWM_TIMER_HW, TCPWM_TIMER_NUM, &TCPWM_TIMER_config) != CY_TCPWM_SUCCESS)
    {
        CY_ASSERT(0);
    }
...
/**********************************************************************************************************************
 * Function Name: isrUserButton
 * Summary:
 *  Interrupt service routine for pressing the user button. Changes the displayed scene.
 * Parameters:
 *  none
 * Return:
 *  none
